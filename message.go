...
func (wac *Conn) Send(msg interface{}) (*proto.WebMessageInfo, error) {
	...
	case response := <-ch:
		...
		if err = json.Unmarshal([]byte(response), &resp); err != nil {
			return nil, fmt.Errorf("error decoding sending response: %v\n", err)
		}
		if int(resp["status"].(float64)) != 200 {
			return nil, fmt.Errorf("message sending responded with %v", resp["status"])
		}
		if int(resp["status"].(float64)) == 200 {
			return msgProto, nil
		}
	case <-time.After(wac.msgTimeout):
		return nil, fmt.Errorf("sending message timed out")
	}
